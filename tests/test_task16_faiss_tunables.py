...
def test_faiss_ef_search_default_and_override(tmp_path):
    ...
    db.upsert([{K_VECTOR: np.random.rand(8).astype(np.float32), K_ID: str(i)} for i in range(20)])
...
def test_faiss_threads_setting(tmp_path):
    ...
    if not hasattr(faiss, "omp_set_num_threads") or not hasattr(faiss, "omp_get_max_threads"):
        pytest.skip("FAISS omp thread APIs not available")

    base = str(tmp_path / "threads")
    db = PicoVectorDB(embedding_dim=8, storage_file=base, faiss_threads=1)
