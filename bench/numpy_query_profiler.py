...
NUM_QUERIES_TO_PROFILE = (
    50  # Number of random queries to run for profiling each scenario
)
K_NEIGHBORS = 10
...
def setup_db(db_size: int, storage_file_base: str) -> PicoVectorDB:
    ...
    print(
        f"Insertion of {db_size:,} vectors took {insert_end_time - insert_start_time:.4f} seconds."
    )
...
def run_query_scenario(
    ...
    results = db.query(
        query_vector, top_k=K_NEIGHBORS, where=where_fn, better_than=better_than_val
    )
    end_time = time.time()
    ...
    print(
        f"  Query time for '{scenario_name}': {end_time - start_time:.6f} seconds. Found {len(results)} results."
    )
    return results
...
def main(db_sizes_to_test: list[int]):
    ...
    for db_size in db_sizes_to_test:
        ...
        try:
            ...
            query_vectors = np.random.rand(NUM_QUERIES_TO_PROFILE, DIMENSION).astype(
                np.float32
            )
            ...
            for i in range(NUM_QUERIES_TO_PROFILE):
                ...
                def simple_where_fn(meta):
                    return meta.get("category_id", -1) % 2 == 0
                run_query_scenario(
                    db,
                    current_query_vector,
                    "With 'where' filter (category_id % 2 == 0)",
                    where_fn=simple_where_fn,
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Profile PicoVectorDB NumPy query path."
    )
    parser.add_argument(
        "--db_sizes",
        type=str,
    ...
    try:
        ...
        print(
            "Error: Invalid format for --db_sizes. Please use comma-separated integers."
        )
        exit(1)
